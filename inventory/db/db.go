...
func ConnectDb() {
	once.Do(func() {
		...
		DB = pool
		log.Println("Connected to db: ecommerce_order_management_system_inventory")
	})
}
...
func CloseDB() {
	if DB != nil {
		DB.Close()
		log.Fatalln("Database connection closed: ecommerce_order_management_system_inventory")
	}
}
